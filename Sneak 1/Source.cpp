...
int main()
{
	...
	do // собственно цикл игры
	{
		...
		if (choose == 'm') // проверяем, была ли нажата какая-либо клавиша и запускаем её обработку в случае ИСТИНЫ
		{
			if (_kbhit())
			{
				...
				case 80: // если была нажата клавиша вниз
					Brain.dy = 1; // то приращение по оси ординат будет положительным
					Brain.dx = 0; // по оси абсцисс приращение нулевое
					break;
				case 72: // если вверх
					Brain.dy = -1; // аналогично согласно геометрической логике
					Brain.dx = 0;
					break;
				case 75: // если влево
					Brain.dy = 0;
		...
		int X = Brain.arrX.back() + Brain.dx; // определяем значение абсциссы головы "змейки" после смещения
		int Y = Brain.arrY.back() + Brain.dy; // то же самое для ординаты
